...
while (res[i] && i < 19) {
    i++;
    res[i] = strtok(NULL, " "); // 다음 단어들(NULL을 넣으면 이전에 저장된 위치에서 부터 계속 잘라냄)
}
```
...
&ensp;fp는 매개변수로 char **를 받고 void를 반환하는 함수를 가리키는 포인터
...
* char **p : "문자열 포인터들의 시작 위치" (예: res는 char*의 배열 → 함수 인자로 받을 땐 char **)
    - 문자열을 여러 개를 가리키는 배열의 첫 번째 주소

| 타입        | 가리키는 대상           | 설명                         |
| --------- | ----------------- | -------------------------- |
| `char *`  | 문자열(문자들의 시작 주소)   | `"apple"`                  |
| `char **` | 문자열 포인터 배열의 시작 주소 | `{"mkdir", "test_folder"}` |
