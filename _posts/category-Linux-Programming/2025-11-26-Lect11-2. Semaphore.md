...
* 여러 프로세스/쓰레드가 같은 자원(공유 변수, 파일, 메모리 등)을 동시에 쓰려고 하면 → 값이 꼬이거나 덮어쓰거나 이상한 결과가 나옴(race condition, 경쟁 상태)
...
&ensp;race condition 예제

&ensp;예: 밥솥에 밥이 공기 남았다.
...
&ensp;결과적으로 밥솥 상태가 꼬임 → race condition(경쟁 상태)


**Race Condition이 왜 발생하는가?**

&ensp;공유 자원을 접근하는 과정은 이렇게 생겼다.

&ensp;**실제 프로그램의 한 줄은 원자적(atomic)이지 않음**
...
&ensp;이 중 어떤 순간에든 다른 프로세스가 끼어들 수 있음

&ensp;그래서 A가 읽고 증가하기 전에 B가 읽고 증가하기 전에 꼬일 수 있음.
...
* 예: 0번 세마포어를 생상자/소비자 버퍼 관리, 1번 세마포어로 다른 자원 관리 
...
## 전체 동작 Flowchart (A/B 둘 다 동일)

&ensp;Step 1. 세마포어(key1) 준비 단계<br/>
```c
key1 = ftok("key", 1)  // Key 생성
semid = semget(key1, 1, IPC_CREAT | IPC_EXCL | 0600)  // 세마포어 생성 시도
```
...
│                                                      │
│  YES (이미 다른 프로세스가 생성함)                       │
│      → semid = semget(key1, 1, 0)   // 그냥 열기      │
│                                                      │
│  NO  (내가 최초 생성자)                                │
│      → arg.val = 0                                   │
│      → semctl(semid, 0, SETVAL, arg) // 초기값 0 설정  │
└──────────────────────────────────────────────────────┘
```

&ensp;결과<br/>
&ensp;A와 B는 동일한 세마포어 ID(semid)를 공유하게 된다. 단, 초기화는 ‘최초 생성자’만 한다.<br/>

&ensp;Step 2. 공유메모리(key2) 준비 단계<br/>
```c
key2 = ftok("key", 2)   // Key 생성
shmid = shmget(key2, sizeof(int)*10, IPC_CREAT | 0600)  // 공유메모리 shmget
```

&ensp;커널 동작<br/>
```objectivec
┌──────────────── 공유메모리(key2) 존재? ───────────────┐
│ YES → 기존 공유메모리의 shmid를 반환                    │
│ NO  → 새로운 공유메모리를 생성 후 shmid 부여             │
└──────────────────────────────────────────────────────┘
```

&ensp;프로세스 주소 공간에 붙이기<br/>
```c
buf = shmat(shmid, 0, 0)
```

&ensp;A와 B는 ‘같은 공유메모리(shmid)’에 붙는다.<br/>

&ensp;Step 3. Writer(A)의 반복루프 (10회)<br/>
```arduino
for i = 0 → 9

    1) scanf("%d", buf+i)    // 공유메모리에 값 저장
    
    2) p_buf.sem_op = +1     // signal
       semop(semid, &p_buf)  // 세마포어 값 증가
...
&ensp;A의 역할:<br/>
* 데이터를 공유메모리에 쓰기
* B가 읽을 수 있도록 signal(+1) 보내기

&ensp;Step 4. Reader(B)의 반복루프 (10회)<br/>
```arduino
for i = 0 → 9

    1) p_buf.sem_op = -1     // wait
       semop(semid, &p_buf)  // 값 1 이상 될 때까지 대기
...
&ensp;이유: 공유메모리 삭제는 보통 Reader가 담당.<br/>
...
└─────┬─────┘                    └─────┬─────┘
 ...
 └──────┬──────┘                      └────┬──────┘
        ...
        A(Writer)                         B(Reader)
   ┌──────────────────┐             ┌──────────────────┐
   │ write buf[i]     │             │ wait sem(-1)     │
   │ signal sem(+1)   │ 10번 반복   │ read buf[i]      │ 10번 반복
   └──────────────────┘             └──────────────────┘
