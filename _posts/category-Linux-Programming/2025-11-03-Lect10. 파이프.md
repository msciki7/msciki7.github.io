...
&ensp;2. 다음은 3개의 child process로부터 message를 전달받아 출력하는 프로그램으로, parent process는 3개의 child process가 모두 종료하면, 통신을 종료한다. 아래 코드에서 ①의 코드를 삭제하고, child process의 종료를 확인하는 대신, timer 설정을 하고, 일정 시간 동안 child process로부터의 message 가 없으면 종료하도록 코드를 수정하시오<br/>

* 원래는 `waitpid(-1, NULL, WNOHANG)`으로 자식 종료를 감지하던 부분을 `select()`의 timeout 기능으로 대체해야 한다.
* while문 구조는 변경하지 않고 ① 부분만 수정
...
void parent(int p[3][2]){
	...
	for(i=0;i<3;i++)
		close(p[i][1]);    // 쓰기 파이프 닫기
	
	FD_ZERO(&master);
	for(i=0;i<3;i++)
		FD_SET(p[i][0], &master);   // 읽기 파이프 3개 등록
	
	while(set=master, select(p[2][0]+1, &set, NULL, NULL, NULL) > 0){
		for(i=0;i<3;i++){
			if(FD_ISSET(p[i][0], &set)){
				n = read(p[i][0], buf, MSGSIZE);
				if(n > 0)
					printf("MSG from %d=%s\n", i, buf);
				else if(n == 0)
					FD_CLR(p[i][0], &master);   // EOF 감지 → FD 제거
			}
		}
		...
		tv.tv_sec = 5;
		tv.tv_usec = 0;
		if(select(p[2][0]+1, &tmp, NULL, NULL, &tv) == 0)
			return;
	}
}
...
int child(int p[2]){
	...
	for(count=0; count<2; count++){
		write(p[1], msg1, MSGSIZE);  // "hello" 전송
		sleep(getpid()%4);           // 랜덤한 시간 대기
	}
	write(p[1], msg2, MSGSIZE);      // bye!! 전송
	close(p[1]);                     // 파이프 닫기 → EOF 전달
	exit(0);
}
```
...
&ensp;1. pipe()와 fork()<br/>
* 부모는 `pipe(pip[i])` 로 3개의 파이프를 만들고
* 각각에 대해 `fork()` 를 수행해 3개의 자식을 생성 → 각 파이프는 한 자식과 부모 간의 통신 통로이다.
    - fork()가 3번 호출되어 자식 3개가 만들어진다.
    - 0이 아니면 부모 프로세스이다.
* 각 자식은 child(pip[i]) 함수를 실행한다.
* 부모는 모든 자식 생성 후 `parent(pip)` 로 넘어간다.

```scss
pip[0] → child #0 ↔ parent  
pip[1] → child #1 ↔ parent  
pip[2] → child #2 ↔ parent
```

| 프로세스 | 역할                  |
| ---- | ------------------- |
| 부모   | 각 자식이 보낸 메시지를 읽고 출력 |
| 자식   | 메시지를 파이프에 씀         |


&ensp;2. child()<br/>
1. 자식은 읽기용 파이프 닫기(`close(p[0])`)
2. "hello" 메시지를 2회 전송(`write(p[1], msg1, MSGSIZE)`)
3. `sleep(getpid() % 4)` → 각 자식이 다른 타이밍으로 실행됨 → 부모가 메시지를 섞어서 받는 효과
4. 마지막에 "bye!!" 메시지 전송 후 `close(p[1])` → EOF 전달

* 자식은 자신이 쓸 파이프의 읽기 쪽(p[0])을 닫고 쓰기 쪽(p[1])으로 메시지를 보낸다.

| 순서 | 동작                    | 설명                     |
| -- | --------------------- | ---------------------- |
| 1  | `"hello"` 2회 write    | MSGSIZE(6) 바이트씩 전송     |
| 2  | `sleep(getpid() % 4)` | 각 자식이 다른 타이밍으로 메시지를 보냄 |
| 3  | `"bye!!"` write       | 마지막 인사 메시지             |
| 4  | `close(p[1])`         | 부모에게 EOF 신호 전달         |
| 5  | `exit(0)`             | 자식 종료                  |

&ensp;이 `close(p[1])` 이 매우 중요한다. 부모가 이제 더 이상 읽을 데이터가 없다. 는 걸 인식하는 선호이다.<br/>

&ensp;3. parent()<br/>
1. 부모는 쓰기용 파이프 닫기(`close(p[i][1])`)
2. 모든 읽기 파이프를 `FD_SET()`으로 master 집합에 등록
3. while문 시작 → `select()`로 자식 중 어느 파이프에서 읽을 데이터가 생겼는지 감시

* 부모는 읽기만 해야 하므로 각 파이프의 쓰기용 FD를 닫는다.
* `select()`로 감시할 파일 디스크립터 집합 master를 초기화하고 각 자식의 읽기 파이프를 등록한다.

&ensp;이제 부모는 세 개의 파이프를 동시에 감시할 준비가 된 상태이다.(select()는 여러 입력 채널을 한꺼번에 기다릴 수 있다.)<br/>

&ensp;부모의 메인 루프<br/>
&ensp;4. select()와 read()<br/>
* `select()`는 읽기 가능 상태의 FD가 생길 때까지 기다린다.
* 데이터가 오면 해당 파이프로부터 `read()` 수행
* "MSG from i=..." 형태로 메시지 출력

&ensp;5. FD_CLR() — 자식 종료 감지<br/>
* 만약 `read()` 결과가 0이라면 (EOF) → 자식이 `close(p[1])` 했다는 뜻 → 더 이상 읽을 데이터가 없으므로 `FD_CLR(p[i][0], &master)`로 감시 대상에서 제거
...
* 5초 동안 추가 메시지가 오지 않으면 `select()` 가 0을 반환
* 부모는 `return` → 통신 종료

| 순서  | 단계                     | 설명                                        |
| --- | ---------------------- | ----------------------------------------- |
| (1) | `select()` 호출          | `master`에 등록된 파이프 중 읽기 가능한 FD가 생길 때까지 기다림 |
| (2) | `read()` 실행            | 데이터가 도착한 파이프로부터 6바이트 읽어 `buf`에 저장         |
| (3) | `printf()`             | “MSG from i=hello” 형태로 출력                 |
| (4) | `read()` 결과가 0인 경우     | 자식이 파이프를 닫은 상태 → `FD_CLR()`로 감시 목록에서 제거   |
| (5) | 두 번째 `select()` (① 부분) | 남은 파이프에서 5초 동안 아무 데이터가 없으면 `return`으로 종료  |


&ensp;프로그램 종료 흐름 요약<br/>

| 단계 | 동작                                             |
| -- | ---------------------------------------------- |
| 1  | 자식들이 각각 메시지를 전송                                |
| 2  | 부모가 select()로 감시하며 메시지 출력                      |
| 3  | 자식이 파이프 닫음 → EOF 감지                            |
| 4  | FD_CLR()로 master 집합에서 제거                       |
| 5  | 모든 파이프가 닫히거나 5초 동안 새 메시지 없음 → select() timeout |
| 6  | 부모 종료 → wait()로 자식 회수 → **쉘로 복귀 ✅**            |
...
&ensp;→ 모든 메시지 출력 후 추가 메시지 없이 5초 경과 → 자동으로 쉘 프롬프트로 복귀<br/>
...
* 세 개의 자식(child 0, 1, 2)이 거의 동시에 첫 번째 "hello"를 보낸다.
* 부모는 `select()` 를 통해 3개의 파이프를 동시에 감시하고 있기 때문에 도착하는 순서대로 출력한다.
* 프로세스 스케줄링 순서에 따라 0, 1, 2 순으로 도착했을 뿐 실행할 때마다 순서는 달라질 수 있다.
...
* 자식 #0이 두 번째 "hello" 를 보내고 바로 "bye!!" 를 이어서 보낸다.
* 자식 #1과 #2 는 `sleep(getpid() % 4)` 로 인해 조금 뒤늦세 "hello" 를 전송한다.
* 이때 부모는 select()로 감시 중이므로 도착한 순서 그대로 차례로 출력한다.
...
* 마지막으로 남은 자식 #1 과 #2rk "bye!!" 를 보내고 close(p[1])로 파이프를 닫는다.
* 부모가 각 자식으로부터 EOF(=read()==0)을 감시하면 FD_CLR()를 실행하여 더 이상 그 파이프를 감시하지 않는다.

&ensp;모든 메시지 송신 완료 후<br/>
* 세 자식이 모두 파이프를 닫으면 더 이상 읽을 데이터가 없고 부모의 master FD 집합은 비어간다.
* 하지만 혹시 남아 있을 수도 있으니 부모는 아래 코드로 추가 메시지가 오는지 5초간 더 기다린다.
...
* select()의 리턴값이 0 → 5초 동안 어떤 자식에서도 메시지 없음 → 부모 return → parent() 종료

&ensp;종료 과정<br/>
1. parent() 함수에서 return
2. main()의 for (i=0; i<3; i++) wait(0); 실행 → 세 자식의 종료를 확인 (좀비 방지)
3. exit(0) 호출 → 프로그램 완전 종료
4. 쉘 프롬프트로 복귀

&ensp;출력 순서가 매번 달라지는 이유<br/>
* sleep(getpid()%4) → 각 자식의 대기 시간이 다름
* 운영체제의 프로세스 스케줄러가 어떤 프로세스를 먼저 실행시킬지 매번 다르기 때문
* 따라서 "MSG from 0=hello"가 먼저 나올 수도, "MSG from 1=hello"가 먼저 나올 수도 있음 → select()는 순서 보장 X, "도착 순서대로" 출력함.

&ensp;핵심 개념 정리<br/>

| 개념                             | 설명                                       |
| ------------------------------ | ---------------------------------------- |
| `pipe()`                       | 프로세스 간 통신(IPC)을 위한 익명 파이프 생성             |
| `fork()`                       | 부모-자식 프로세스 복제                            |
| `select()`                     | 여러 FD를 동시에 감시(읽기/쓰기/예외)                  |
| `FD_SET`, `FD_CLR`, `FD_ISSET` | select()와 함께 FD 집합 관리                    |
| `timeval` 구조체                  | select()의 대기 시간 설정 (`tv_sec`, `tv_usec`) |
| `read() == 0`                  | EOF (자식이 파이프 닫음)                         |
...
&ensp;3개의 자식이 각각 메시지를 파이프로 보내고 부모는 `select()` 를 이용해 모든 파이프를 동시에 감시하다가 5초 동안 메시지가 없으면 자동 종료하는 프로그램이다.<br/>

FIFO
=====

&ensp;FIFO(First In First Out)는 이름이 있는 (named) 파이프로 서로 다른 프로세스들 간 통신이 가능한 IPC(Inter-Process Communication)방법이다.<br/>

&ensp;pipe vs FIFO 비교<br/>

| 구분         | pipe                    | FIFO                                        |
| ---------- | ----------------------- | ------------------------------------------- |
| 연결 가능 프로세스 | 부모–자식 관계만 가능 (공통 조상 필요) | 완전히 독립된 프로세스 간 통신 가능                        |
| 파일 이름      | 없음 (익명)                 | 있음 (파일시스템에 존재)                              |
| 생성 방식      | `pipe(int fd[2])`       | `mkfifo(const char *pathname, mode_t mode)` |
| 사용 방식      | `read()`, `write()`     | 동일하게 `open()`, `read()`, `write()` 가능       |
| 접근 권한      | 메모리 내 버퍼                | 일반 파일처럼 소유자, 권한 존재                          |

&ensp;특징<br/>
* UNIX 파일처럼 이름이 부여됨
* 소유자, 크기, 접근 권한 등 일반 파일 속성을 가짐
* open, close, read, write, remove 같은 일반 파일 연산 가능

&ensp;pipe는 부모-자식 간 통신만 가능하지만 FIFO는 파일 이름이 있어 완전히 독립된 프로세스 간에도 통신 가능하다.<br/>
...
&ensp;이렇게 하면 파일 시스템에 "fifo_file" 이라는 이름의 FIFO 파일이 생성됨 ㅣㅇ후 일반 파일처럼 open() 해서 read() 또는 write() 가능<br/>

&ensp;FIFO 열기(open)<br/>
* 읽기 전용으로 열기: `open("fifo_file", O_RDONLY);`
* 쓰기 전용으로 열기: `open("fifo_file", O_WRONLY);`
* 읽기/쓰기 겸용으로 열기: `open("fifo_file", O_RDWR);`

&ensp;`O_RDWR` 의 의미<br/>
&ensp;일반적으로 FIFO는 한쪽이 읽기, 한쪽이 쓰기를 담당하만 특정 상황(예: 단일 프로세스 테스트나 blocking 방지)에서는 한 프로세스가 양방향 모드(O_RDWR)로 열기도 한다.<br/>
...
| 상황                    | 설명                                        |
| --------------------- | ----------------------------------------- |
| **기본(open)**          | 반대편 프로세스가 아직 열지 않았으면 **blocking** (대기 상태) |
| **non-blocking open** | 반대편이 준비되지 않았다면 `-1` 반환, `errno = ENXIO`   |

&ensp;FIFO는 한쪽이 open()할 때 상대 프로세스가 연결될 때까지 기다린다.<br/>

&ensp;예시 흐름<br/>
1. 서버(Reader): `open("fifo_file", O_RDONLY);`
2. 클라이언트(Writer): `open("fifo_file", O_WRONLY);`
3. 클라이언트가 데이터를 `write()`, 서버가 `read()`로 읽음

&ensp;Non-blocking 모드의 예시<br/>
```c
int fd = open("fifo_file", O_RDONLY | O_NONBLOCK);
if (fd == -1 && errno == ENXIO)
    printf("상대 프로세스가 아직 준비되지 않았습니다.\n");
```

&ensp;Reader가 O_RDWR로 여는 이유<br/>
* Writer가 먼저 종료되면 Reader는 더 이상 데이터를 받을 수 없고 EOF(0)를 받음
* O_RDWR로 열면 Reader 자신ㅇ이 write end를 함께 열어둬서 blocking 없이 계속 대기할 수 있다.

&ensp;정리<br/>

| 항목                        | 설명                             |
| ------------------------- | ------------------------------ |
| **Blocking open**         | 상대가 열기 전까지 대기                  |
| **Non-blocking open**     | 상대가 없으면 즉시 -1, `errno = ENXIO` |
| **Reader가 O_RDWR로 여는 이유** | Writer 종료 후에도 blocking 방지      |
| **FIFO 동작 방식**            | 한쪽에서 쓰면 다른 쪽에서 읽음 (FIFO 순서 유지) |


# FIFO를 이용한 클라이언트–서버 통신 예제

&ensp;전체 개요<br/>
&ensp;mkfifo()로 "fifo" 라는 이름의 FIFO 파일을 생성하고 서버는 FIFO를 통해 데이터를 읽기만 클라이언트는 FIFO에 데이터를 쓰기만 한다.<br/>

&ensp;서버는 데이터를 실시간으로 표준 출력(stdout)에 보여주며 클라이언트는 키보드 입력(stdin)을 받아 FIFO로 전송한다.<br/>
...
int main(void) {
    ...
    mkfifo("fifo", 0600);       // FIFO 생성 (rw-------)

    fd = open("fifo", O_RDWR);  // 읽기/쓰기 겸용으로 열기

    for (;;) {                  // 무한 루프
        n = read(fd, buf, 512); // FIFO로부터 데이터 읽기
        write(1, buf, n);       // 표준 출력(1번 fd)에 출력
    }
}
```
...
| 부분                      | 설명                                              |
| ----------------------- | ----------------------------------------------- |
| `mkfifo("fifo", 0600);` | `"fifo"`라는 이름의 FIFO 파일 생성. 0600 → 소유자만 읽기/쓰기 가능 |
| `open("fifo", O_RDWR);` | 읽기/쓰기 겸용으로 열어 **blocking 방지**                   |
| `read(fd, buf, 512);`   | FIFO로부터 데이터 읽음 (클라이언트가 write할 때까지 대기)           |
| `write(1, buf, n);`     | 읽은 데이터를 화면(표준 출력)으로 출력                          |

&ensp;핵심 포인트<br/>
&ensp;`O_RDWR`로 연 이유는 writer(클라이어트)가 아직 FIFO를 열지 않았을 때 read()가 blocking되지 않게 하기 위해서이다.

&ensp;서버가 먼저 실행되어도 멈추지 않고 대기할 수 있다.<br/>
...
&ensp;서버는 클라이언트가 보낸 데이터를 그래도 터미널에 출력한다.<br/>
...
int main(void) {
    ...
    for (j = 0; j < 3; j++) {
        nread = read(0, buf, 512);   // 표준 입력(stdin)에서 데이터 읽기
        write(fd, buf, nread);       // FIFO에 데이터 쓰기
    }
...
| 부분                      | 설명                            |                              |
| ----------------------- | ----------------------------- | ---------------------------- |
| `open("fifo", O_WRONLY  | O_NONBLOCK)`                  | FIFO를 **쓰기 전용 + 논블로킹 모드**로 엶 |
| `read(0, buf, 512)`     | 키보드(표준 입력)에서 문자열 입력 받음        |                              |
| `write(fd, buf, nread)` | 입력받은 데이터를 FIFO에 씀 (→ 서버로 전달됨) |                              |
| `for (j=0; j<3; j++)`   | 3회 반복 입력 후 종료                 |                              |

&ensp;Non-blocking 의미<br/>
&ensp;일반적으로 FIFO를 O_WRONLY로 열면 reader(서버)가 아직 열지 않은 상태에서는 `open()`이 blocking(대기)된다.<br/>
&ensp;`O_NONBLOCK` 을 함께 지정하면 서버가 준비되지 않은 경우 즉시 `open failed` 를 출력하고 종료한다.<br/>

&ensp;전체 통신 흐름<br/>

| 단계 | 서버 동작                                | 클라이언트 동작          |               |
| -- | ------------------------------------ | ----------------- | ------------- |
| 1  | `mkfifo()`로 FIFO 생성                  | (없음)              |               |
| 2  | `open("fifo", O_RDWR)`로 FIFO 열기 (대기) | (아직 실행 안 함)       |               |
| 3  | 클라이언트 실행 → `open("fifo", O_WRONLY    | O_NONBLOCK)`      | 서버 쪽 FIFO 연결됨 |
| 4  | 클라이언트가 입력한 문자열을 `write()`            | 서버는 `read()`로 수신  |               |
| 5  | 서버는 수신 데이터를 터미널에 출력                  | 클라이언트는 3회 입력 후 종료 |               |
...
| ----------- | ---------------------- | ------------- | ----------- |
| FIFO 생성     | `mkfifo("fifo", 0600)` | -             |             |
| open 방식     | `O_RDWR` (blocking 방지) | `O_WRONLY     | O_NONBLOCK` |
| 읽기/쓰기 대상    | FIFO → stdout          | stdin → FIFO  |             |
| 실행 순서       | 먼저 실행                  | 나중 실행         |             |
| Blocking 여부 | 없음                     | 없음            |             |
| 통신 방향       | 클라이언트 → 서버 (단방향)       |               |             |

&ensp;이 예제는 FIFO(named pipe)를 이용한 가장 기본적인 클라이언트-서버 통신 구조이다. 서버는 `mkfifo()`로 FIFO를 만들고 `O_RDWR` 모드로 열어 대기하며 클라이언트는 `O_WRONLY | O_NONBLOCK` 모드로 열어 데이터를 보낸다. 서버는 FIFO를 통해 전달된 데이터를 읽어 표준 출력에 표시한다.<br/>
&ensp;이를 통해 서로 독립된 프로세스 간 통신(IPC)이 가능하며 소켓 통신으로 확장되는 개념의 기초가 된다.<br/>

실습 2
=====

&ensp;1. fifo를 이용하여 통신하는 두 개의 프로그램을 작성합니다. 프로그램 A는 외부 입력으로 정수를 입력받아 프로그램 B에 전달합니다. 프로그램 B는 전달받은 정수에 +8을 한 뒤 프로그램 A에 돌려줍니다. 프로그램 A는 돌려받은 정수값을 출력합니다. -1이 입력되면 두 프로그램은 종료합니다. <br/>
...
&ensp;동작 메커니즘 (FIFO 양방향 통신)<br/>
&ensp;1) 준비 단계<br/>
* 프로그램 A
    - `mkfifo("f1")`, `mkfifo("f2")`로 이름 있는 파이프(FIFO) 2개를 만듦
    - f1을 쓰기 전용(O_WRONLY) 으로, f2를 읽기 전용(O_RDONLY) 으로 open
* 프로그램 B
    - f1을 읽기 전용, f2를 쓰기 전용으로 open
* 주의: O_WRONLY로 연 FIFO는 반대편이 열릴 때까지 블로킹됨 그래서 보통 B를 먼저 실행하면 A가 바로 붙음

&ensp;2) 데이터 흐름<br/>
```scss
A(stdin) --정수--> f1 ----> B(read) --(+8)--> f2 ----> A(read)
```

* A: scanf로 정수 in 입력 → `write(fd[0], &in, sizeof(int))`로 f1에 이진 형태(int 크기)로 보냄
* B: `read(fd[0], &in, sizeof(int))`로 f1에서 수신 → `int += 8` 계산 → `write(fd[1], &in, sizeof(int))` 로 f2로 전송
* A: `read(fd[1], &in, sizeof(int))` 로 f2에서 결과 수신 → printf로 출력

&ensp;FIFO는 반이중(half-duplex)이라 한 방량당 하나씩 총 2개(f1, f2)를 써서 양방향을 만듦<br/>

&ensp;3) 종료 프로토콜<br/>
* A가 -1 입력 → `write(f1, -1)` 전송 → A는 즉시 exit(0) (되돌려받지 않음)
* B는 f1에서 -1을 읽으면 즉시 `exit(0)` ⇒ 두 프로그램 모두 정상 종료

&ensp;4) 블로킹/원자성 포인터<br/>
* `read` / `write` 는 해당 FIFO의 커널 버퍼를 통해 블로킹 동작
* `sizeof(int)` 단위 전송은 작고 원자적(PIPE_BUF 이하)이라 깨지고 않고 그대로 한 덩어리로 전달됨
* 텍스트가 아니라 바이너리 int로 주고받으므로 양쪽 모두 `sizeof(int)`로 맞춰야 함

&ensp;5) 왜 B에서 write(fd[1], …)가 정답인가?<br/>
* 통신 방향이 f1: A→B, f2: B→A로 고정
* 따라서 B에서 결과는 반드시 fd[1](= f2, 쓰기용) 로 보내야 A가 read(fd[1], …)로 받음
* fd[i]처럼 인덱스 변수를 쓰면 값이 보장되지 않아 엉뚱한 디스크립터로 쓰게 되어 결과가 안 돌아옴

&ensp;FIFO 통신 구조 (프로그램 A ↔ 프로그램 B)<br/>
```scss
 ┌─────────────────────────────┐                     ┌────────────────────────────┐
 │           프로그램 A         │                     │          프로그램 B         │
 │─────────────────────────────│                     │────────────────────────────│
 │  scanf() → 사용자 입력       │                     │                            │
 │       ↓                     │                     │                            │
 │  write(fd[0], &in) ────────▶│──── f1 ───▶ [커널 FIFO 버퍼] ───▶ read(fd[0], &in) │
 │       ↑                     │                     │                            │
 │  read(fd[1], &in) ◀─────────│──── f2 ◀─── [커널 FIFO 버퍼] ◀─── write(fd[1], &in)│
 │       │                     │                     │                            │
 │  printf("%d", in); 출력      │                     │   in = in + 8; 계산         │
 │─────────────────────────────│                     │────────────────────────────│
 │     -1 입력 시 exit(0)      │                     │  -1 읽으면 exit(0)         │
 └─────────────────────────────┘                     └────────────────────────────┘
```

&ensp;데이터 흐름 단계별 설명<br/>
1. A → f1 → B
* A가 입력받은 정수를 f1(쓰기용)에 보냄
* B는 f1(읽기용)에서 정수를 수신
2. B의 처리
* 받은 값에 +8을 더함
3. B → f2 → A
...
* f1: A → B 방향
* f2: B → A 방향
* FIFO는 커널이 관리하는 버퍼이므로, 두 프로세스가 서로 파일처럼 읽고 쓴다.
* sizeof(int) 단위로 이진 데이터 전송 → 깨지지 않고 그대로 전달됨.
* -1은 종료 신호로 동작