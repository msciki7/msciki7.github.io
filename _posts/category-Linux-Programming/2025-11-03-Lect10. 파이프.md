...
* 자식 #1과 #2 는 `sleep(getpid() % 4)` 로 인해 조금 뒤늦게 "hello" 를 전송한다.
...
* 마지막으로 남은 자식 #1 과 #2가 "bye!!" 를 보내고 close(p[1])로 파이프를 닫는다.
...
&ensp;이렇게 하면 파일 시스템에 "fifo_file" 이라는 이름의 FIFO 파일이 생성됨 이후 일반 파일처럼 open() 해서 read() 또는 write() 가능<br/>
...
&ensp;`O_RDWR` 의 의미<br/>
&ensp;일반적으로 FIFO는 한쪽이 읽기, 한쪽이 쓰기를 담당하면 특정 상황(예: 단일 프로세스 테스트나 blocking 방지)에서는 한 프로세스가 양방향 모드(O_RDWR)로 열기도 한다.<br/>
...
* O_RDWR로 열면 Reader 자신이 write end를 함께 열어둬서 blocking 없이 계속 대기할 수 있다.
...
&ensp;FIFO는 반이중(half-duplex)이라 한 방향당 하나씩 총 2개(f1, f2)를 써서 양방향을 만듦<br/>
