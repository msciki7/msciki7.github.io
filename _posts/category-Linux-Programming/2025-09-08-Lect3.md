...
* 666 → rw-rw-rw-와 같은 의미(소유자/그룹/기타 모두 읽기·쓰기 가능). ls -l data1로 보면 -rw-rw-rw- 비슷하게 보일 거다.
* Link count: 1 -> 일반 파일은 기본적으로 1
* File size: 12bytes -> 파일 안에 글자 12개 정도가 들어 있다는 뜻
...
```c
#include <stdio.h>   // printf 사용
#include <unistd.h>  // access 함수와 F_OK, R_OK, W_OK 상수

int main(void) {
    // 파일 존재 확인
    if (access("data1", F_OK) == 0) {
        // 읽기 + 쓰기 가능 확인 (두 조건을 합칠 때 | 사용)
...
&ensp;결과<br/>
&ensp;파일이 있고 읽기/쓰기 둘 다 가능한 상태일 때<br/>
```bash
File exists: Read/write operations are permitted.
```

&ensp;읽기만 가능할 때<br/>
```bash
File exists: Read/write operations are not permitted.
```
...
# 모두 읽기/쓰기
chmod 666 data1
./access_test   # → permitted

# 모두 읽기만
chmod 444 data1
./access_test   # → not permitted

# 파일 삭제 후
rm data1
./access_test   # → No such file or directory ...
```

<p align="center"><img src="/assets/img/Linux Programming/Lect3/3-3.png" width="600"></p>
