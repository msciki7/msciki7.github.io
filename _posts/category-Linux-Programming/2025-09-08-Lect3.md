...
#include <sys/types.h>    // stat에 필요한 타입
#include <sys/stat.h>     // struct stat 정의
#include <unistd.h>       // stat 함수 선언

int main(void) {
    struct stat buf;            // 파일 정보를 담을 상자(자료 구조) 하나 준비

    stat("data1", &buf);        // "data1"의 정보를 buf 상자에 채워 넣기

    // 접근 권한(맨 끝 3자리만 필요해서 & 0777), 링크 수, 파일 크기 출력
...
echo "hello world" > p1.c
```

&ensp;1) 하드 링크 만들기: link()<br/>
&ensp;(1) 코드: link_test.c<br/>
```c
#include <stdio.h>
#include <unistd.h>
...
&ensp;코드 설명<br/>
* scanf -> 원본 파일 이름, 새 하드링크 이름을 입력받음
* link(src, dst) -> 원본 파일과 똑같은 하드링크를 생성
...
&ensp;결과 확인<br/>
```bash
9241030 -rw-r--r--. 2 ec2-user ec2-user    12 Oct 19 09:01 abc
9241027 -rwxr-xr-x. 1 ec2-user ec2-user 16752 Oct 19 08:38 access_test
9241028 -rw-r--r--. 1 ec2-user ec2-user   469 Oct 19 08:37 access_test.c
9241026 -rw-r--r--. 1 ec2-user ec2-user    12 Oct 19 08:23 data1
9241029 -rwxr-xr-x. 1 ec2-user ec2-user 16752 Oct 19 09:03 link_test
9241031 -rw-r--r--. 1 ec2-user ec2-user   145 Oct 19 09:02 link_test.c
9241030 -rw-r--r--. 2 ec2-user ec2-user    12 Oct 19 09:01 p1.c
9241024 -rwxr-xr-x. 1 ec2-user ec2-user 16744 Oct 19 08:23 stat_test
9241025 -rw-r--r--. 1 ec2-user ec2-user   305 Oct 19 08:19 stat_test.c
```

* abc와 p1.c는 같은 inode 번호를 가짐
* 링크 카운트(2)가 늘어남 -> 하나의 파일을 가리키는 이름이 2개가 됨
* 원본을 지워도 다른 링크가 남아있으면 파일 내용은 살아 있음

&ensp;개념<br/>
* inode: 파일의 실제 내용과 메타데이터가 저장되는 공간
* 하드링크: 같은 inode를 가리키는 또 다른 이름
...
&ensp;2번 코드: symlink_test.c (심볼릭링크 만들기)<br/>
```c
#include <stdio.h>
#include <unistd.h>
...
&ensp;구조 설명<br/>
* symlink(src, dst) -> 원본 파일 경로만 가리키는 심볼릭 링크 생성
...
9241028 -rw-r--r--. 1 ec2-user ec2-user   469 Oct 19 08:37 access_test.c
9241026 -rw-r--r--. 1 ec2-user ec2-user    12 Oct 19 08:23 data1
9241035 lrwxrwxrwx. 1 ec2-user ec2-user     4 Oct 19 09:10 def -> p1.c
9241029 -rwxr-xr-x. 1 ec2-user ec2-user 16752 Oct 19 09:03 link_test
9241031 -rw-r--r--. 1 ec2-user ec2-user   145 Oct 19 09:02 link_test.c
9241030 -rw-r--r--. 2 ec2-user ec2-user    12 Oct 19 09:01 p1.c
9241024 -rwxr-xr-x. 1 ec2-user ec2-user 16744 Oct 19 08:23 stat_test
9241025 -rw-r--r--. 1 ec2-user ec2-user   305 Oct 19 08:19 stat_test.c
9241032 -rwxr-xr-x. 1 ec2-user ec2-user 16760 Oct 19 09:10 symlink_test
9241033 -rw-r--r--. 1 ec2-user ec2-user   148 Oct 19 09:09 symlink_test.c
```

* 실제로는 def파일 안에 "p1.c"라는 문자열이 저장돼 있음
* l → 이 파일은 심볼릭 링크라는 뜻
...
&ensp;2) 디렉토리 A/B/C를 가리키는 심볼릭 링크 T 만들기<br/>
&ensp;디렉토리는 하드링크가 거의 금지라서 반드시 symlink를 써야 한다.<br/>
```bash
ln -s A/B/C T
```

&ensp;3) 심볼릭 링크 T를 통해 파일 생성<br/>
```bash
echo "hello" > T/data2      # 내용까지 넣고 싶으면
# 또는 내용 없이 비어있는 파일만 만들려면
# touch T/data2
```
...
ls -l A/B/C
```
* ls -l 출력에서 T -> A/B/C 형태로 보이면 symlink가 제대로 됨.
* T/data2와 A/B/C/data2가 동일 파일로 보일 것(경로만 다르고 실제 위치는 같음)

&ensp;결과<br/>
```bash
ls -l
drwxr-xr-x. 3 s23013240 class 4096 2025-09-10 18:10 A 
lrwxrwxrwx. 1 s23013240 class 5 2025-09-10 18:10 T -> A/B/C
-rw-r--r--. 2 s23013240 class   12 2025-09-10 18:02 abc
lrwxrwxrwx. 1 s23013240 class    4 2025-09-10 18:08 def -> p1.c
-rwxr-xr-x. 1 s23013240 class 6743 2025-09-10 18:01 link_test
-rw-r--r--. 1 s23013240 class  148 2025-09-10 18:01 link_test.c
-rw-r--r--. 2 s23013240 class   12 2025-09-10 18:02 p1.c
-rwxr-xr-x. 1 s23013240 class 6749 2025-09-10 18:08 symlink_test
-rw-r--r--. 1 s23013240 class  152 2025-09-10 18:07 symlink_test.c
...
ls -l A/B/C
합계 4
-rw-r--r--. 1 s23013240 class 6 2025-09-10 18:10 data2
```

* 심볼릭 링크는 디렉토리에도 사용가능
* 경로를 따라가기 때문에 T를 통해서도 A/B/C 안에 조작할 수 있음
...
int main(void) {
    ...
    printf("%s: perm=%o, size=%ld bytes\n",
           linkname, st_link.st_mode & 0777, (long)st_link.st_size);

    stat(linkname, &st_target);
    printf("-> target: perm=%o, size=%ld bytes\n",
           st_target.st_mode & 0777, (long)st_target.st_size);
...
* stat(path, &buf) -> 심볼릭 링크를 따라가서 “진짜 타깃” 파일의 정보를 채움.(ex. def가 p1.c를 가리키면 p1.c 정보를 채움) 즉 링크가 가리키는 실제 파일의 정보.
* lstat(path, &buf) -> 링크를 따라가지 않고, “링크 자체”의 정보를 채움(ex. def 자체가 링크 파일임을 알 수 있고, st_size는 "p1.c" 글자 수). 즉 링크 자체의 정보.
    - 링크 자체를 보고 싶다 → lstat
...
<p align="center"><img src="/assets/img/Linux Programming/Lect3/3-2.png" width="600"></p>

&ensp;📝 전체 개념 요약<br/>
1. 하드링크 (link)
* 원본과 같은 inode 공유.
* 원본 지워도 다른 링크가 있으면 데이터는 유지.
2. 심볼릭 링크 (symlink)
* 원본 경로 문자열만 저장.
* 원본이 없어지면 깨진 링크.
* 디렉토리에도 사용 가능.
3. stat vs lstat
* stat: 링크를 따라가 실제 파일 확인.
* lstat: 링크 자체 확인.