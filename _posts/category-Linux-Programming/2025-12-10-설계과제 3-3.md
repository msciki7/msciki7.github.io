...
typedef struct {
    int next_msg_num;
    int active_users;
    int user_alive[MAX_USERS+1];
} SharedData;

SharedData *shm;
int shmid, msgq_id, semid;
...
// ================= 사용자 등록 =================
int assign_id(int *start_seq) {
    P();

    if (shm->next_msg_num == 0) {
        shm->next_msg_num = 1;
        shm->active_users = 0;
...
// ================= 사용자 종료 =================
void unregister_user(int my_id) {
    P();
    shm->user_alive[my_id] = 0;
    shm->active_users--;
    int remain = shm->active_users;
    V();
...
void sender(int my_id) {
    ...
    while (1) {
        ...
        if (strcmp(buf, "talk_quit") == 0) {
            printf("[User %d] sender exit.\n", my_id);
            break;
        }

        int seq = get_next_msg_num();

        int alive[MAX_USERS+1];
        P();
        for (int i = 1; i <= MAX_USERS; i++)
            alive[i] = shm->user_alive[i];
        V();

        for (int uid = 1; uid <= MAX_USERS; uid++) {
            if (!alive[uid]) continue;

            Message msg;
            msg.mtype = seq * 10 + uid;
            msg.sender = my_id;
...
// ================= receiver =================
void receiver(int my_id, int start_seq, pid_t sender_pid) {
    int next_seq = start_seq;

    while (1) {
        // 1) sender가 죽었는지 확인
        if (kill(sender_pid, 0) == -1) {
            // sender 종료 → receiver도 종료
            break;
        }

        long type = next_seq * 10 + my_id;
        Message msg;

        if (msgrcv(msgq_id, &msg, MSG_SIZE, type, IPC_NOWAIT) >= 0) {
            printf("[msg#%d sender=%d] %s\n", next_seq, msg.sender, msg.text);
            next_seq++;
        }

        // 2) 모든 사용자 종료 → receiver도 종료
        P();
        int alive = shm->active_users;
        V();
        if (alive == 0)
            break;
...
// ================= IPC 초기화 =================
void init_ipc() {
    key_t k1 = ftok("msgkey", 'M');
    msgq_id = msgget(k1, IPC_CREAT | 0666);

    key_t k2 = ftok("msgkey", 'S');
    shmid = shmget(k2, sizeof(SharedData), IPC_CREAT | 0666);
    shm = (SharedData*)shmat(shmid, NULL, 0);

    key_t k3 = ftok("msgkey", 'E');
    semid = semget(k3, 1, IPC_CREAT | 0666);
...
int main() {
    ...
    int start_seq;
    int my_id = assign_id(&start_seq);

    if (my_id < 0) {
        printf("[SYSTEM] 최대 %d명이 이미 참여 중입니다.\n", MAX_USERS);
        printf("[SYSTEM] 접속을 거절합니다.\n");
        return 1;
    }

    printf("User %d joined. (start_seq=%d)\n", my_id, start_seq);
    ...
    if (receiver_pid == 0) {
        receiver(my_id, start_seq, sender_pid);
        exit(0);
    }
