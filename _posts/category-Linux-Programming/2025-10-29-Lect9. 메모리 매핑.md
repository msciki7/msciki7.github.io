...
&ensp;프로그램 요약<br/>
* writer: 자신의 PID를 유 파일(data)에 써서 알리고, SIGUSR1 시그널을 받을 때마다 temp에 메모리 매핑된 내용을 표준출력으로 내보냅니다(3회)
...
&ensp;전체 동작 타임라인<br/>
1. writer 시작
* data 파일을 `mmap(PROT_WRITE | MAP_SHARED)` 히여 `pid` 위치를 공유 매모리처럼 쓴다.
* 자신의 PID를 `*pid` 에 기록한다.
...
* 두 파일의 크기를 미리 확보해둠
* 이제 data는 pid크기 만큼, temp는 512바이트 크기 확보
...
* 두 파일의 크기를 미리 확보해둠
* 이제 data는 pid 크기만큼 temp는 512바이트 크기 확보
