...
void run_server_and_user1() {
    ...
    if (fd21 < 0) { perror("open fifo_2_1"); exit(1); }

    int fd31 = open(f31, O_RDONLY | O_NONBLOCK);
    if (fd31 < 0) { perror("open fifo_3_1"); exit(1); }
    ...
    while (1) {
        ...
        if (FD_ISSET(STDIN_FILENO, &set)) {
            ...
            /* 자기 화면에 출력 */
            printf("[sender=1 & msg#=%d] %s", global_seq, line);
            fflush(stdout);
            ...
            for (int to = 2; to <= 3; to++) {
                ...
                snprintf(out, sizeof(out),
                        "[sender=1 & msg#=%d] %s", global_seq, line);
        ...
        if (FD_ISSET(fd21, &set)) {
            ...
            if (n > 0) {
                ...
                for (int to = 2; to <= 3; to++) {
                    ...
                    snprintf(out, sizeof(out),
                            "[sender=2 & msg#=%d] %s", global_seq, buf);
                    write(fd, out, strlen(out));
                    close(fd);
                }
            }
        ...
        if (FD_ISSET(fd31, &set)) {
            ...
            if (n > 0) {
                ...
                for (int to = 2; to <= 3; to++) {
                    ...
                    snprintf(out, sizeof(out),
                            "[sender=3 & msg#=%d] %s", global_seq, buf);
                    write(fd, out, strlen(out));
                    close(fd);
                }
            }
...
void run_client(int myid) {
    ...
    /* 서버 -> 나 */
    make_name(fin, 1, myid);
    int fd_in = open(fin, O_RDONLY | O_NONBLOCK);
    if (fd_in < 0) { perror("open fifo_1_x"); exit(1); }

    /* 내가 -> 서버 */
    make_name(fout, myid, 1);
...
### message queue를 이용하여 talk 프로그램을 구현하는 경우 필요한 message queue의 개를 쓰시오
...
struct message {
    int msg_no;        // 전역 메시지 번호
    int sender;        // 보낸 사용자 id (1~3)
    char text[MAXTEXT];
};

/* --- 공유 메모리 구조 --- */
struct shm_area {
    int init_done;     // 1번 프로세스가 초기화 했는지
    int global_msg_no; // 마지막으로 기록된 메시지 번호
    struct message log[MAXMSG];
};

/* --- 세마포어 P, V --- */
void P(int semid, int idx) {
    struct sembuf p = { idx, -1, 0 };
...
void run_user(int myid, struct shm_area *shm, int semid) {
    int next_no = 1;        // 내가 다음에 읽어야 할 msg_no
    char line[MAXTEXT];
    ...
    while (1) {
        ...
        // global_msg_no 스냅샷 획득
        P(semid, 0);
        snapshot = shm->global_msg_no;
        V(semid, 0);

        // next_no ~ snapshot까지 출력
