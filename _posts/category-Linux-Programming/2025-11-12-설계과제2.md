...
&ensp;이 과제는 프로그램 실행 도중에 비정상 종료가 발생해도 다시 실행했을 때 이전 작업을 이어서 수행할 수 있는 복구 가능한 프로그램(recoverable program)을 만드는 것이 목표이다. 이를 위해 복구 함수를 작성하고 로컬 파일을 체크 포인트처럼 활용해 프로그램의 진행 상태를 저장한다.<br/>
...
&ensp;System V IPC 버전 MYIO3.H<br/>
...
static DataUnion *addr1;   // 입력 로그
static int *addr2;         // 카운터 로그
...
static inline void r_init() {
    ...
    shmid1 = shmget(key1, LOG1_SIZE, IPC_CREAT | 0600);
    shmid2 = shmget(key2, LOG2_SIZE, IPC_CREAT | 0600);
    ...
    addr1 = (DataUnion *)shmat(shmid1, NULL, 0);
    addr2 = (int *)shmat(shmid2, NULL, 0);
    ...
    while (1) {
        ...
        if (pid == 0) {
            ...
            if (mode == 1 && nscanf > 0) {
                ...
                if (target > nscanf) target = nscanf;
                nscanf = target;
                addr2[1] = nscanf;
...
&ensp;1. 기존 방식(1·2단계)의 한계: 복구 비용이 O(N)
