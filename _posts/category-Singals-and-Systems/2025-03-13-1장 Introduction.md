...
&ensp;-연속신호 : $x(t) = x(t + T)$ (T: 기본주기), 주파수 : $f = 1/T (T = 2π/ω)$
&ensp;-이산신호 : $x[n] = x[n + N]$ (N: 기본 주기기) , 주파수 : $ω = 2πf = 2π/N$<br/>
이산시간 신호는 연속시간 신호를 일정한 비율로 샘플링하여 얻는다. (샘플링(sampling): 메시지 신호를 특정 순간에서 그 진폭을 나타내는 일련의 수로 변환환)

&ensp;2. 우함수 신호와 기함수 신호(Even and Odd signals)
&ensp;우함수 신호 : $x(-t) = x(t)$<br/>
기함수 신호: $x(-t) = -x(t)$
&ensp;우함수 신호는 수직축에 대하여 수직, 기함수 신호는 원점에 대하여 대칭
...
&ensp;컬레 대칭(conjugate symmetry)
<center>$x(-t) = x*(t)$</center><br/>
<center>$x(t) = a(t) + ib(t)$</center><br/>
<center>$x*(t) = a(t) - ib(t)$</center><br/>
<center>$a(-t) + ib(-t) = a(t) - ib(t)$</center><br/>
&ensp;its real part is even, its imaginary part is odd(실수부 - 우함수 신호, 허수부 - 기함수 신호)


&ensp;3. 주기 신호와 비주기 신호(Periodic and aperiodic signals)
&ensp;$x(t) = x(t + T)$ (모든 t에 대하여)<br/> 
T는 x(t)의 기본주기(fundamental period); x(t)의 기본 주파수<br/>
$f = 1/T$ (주파수 f의 단위: 헤르츠(Hz))<br/>
단위가 rad/sec인 각주파수(angular frequency) -> $ω = 2\pi f = 2\pi / T$

&ensp;비주기적 신호는 만족하는 T가 존재하지 않는 신호
...
$x[n] = x[n+N]$ (n이 정수일 경우), $Ω = 2π/N$ (단위 : rad/smple)
 
&ensp;4. 결정적 신호, 랜덤신호(deterministic signal, random signal)
&ensp;결정적 신호 : 어느 시간에서의 값이 불확실성이 없는 신호
&ensp;랜덤신호 : 발생 이전에 불확실성이 있는 신호, 그룹에 있는 각 신호는 특정한 발생 확률(probability of occurrence)을 가짐, 그러한 신호의 전체를 랜덤 프로세스(random process)라고 함.

--------

&ensp;에너지 신호, 전력신호(Energe signal, power signal)
&ensp;순시전력 : $p(t) = \nu ^{2}(t)/R or p(t) = R\imath ^{2}(t)$
&ensp;순시 전력 p(t)는 신호의 진폭의 제곱에 비례 특히 1\Omega저항인 경우 위에 식 같은 형태가 됨
&ensp;normalized power($R = 1\Omega$) : $p(t) = x ^{2}(t)$
&ensp;연속시간 신호 x(t)의 전체 에너지
<p align="center"><img src="/assets/img/Singals and Systems/1장 Introduction/1-6.jpg" width="600"></p>
&ensp;평균전력 기본주기가 T인 주기신호 $x(t) : P = \frac{1}{T}\int_{T/2}^{-T/2}x^{2}(t)dt$
&ensp;평균전력 P의 제곱근 = 신호 x(t)의 실효값(root-mean-square, rms)
&ensp;이산신호 x[n]의 전체 에너지 $E = \sum_{n=-\infty }^{\infty }x^{2}[n]$
&ensp;기본주기 가 N인 주기 신호 x[n]의 평균 전력 $P = \frac{1}{N}\sum_{n=-N}^{N}x^{2}[n]$<br/>
0<E<∞를 만족시키면 에너지 신호, 0<P<∞를 만족시키면 

&ensp;신호에 대한 기본적인 연산
&ensp;진폭 스케일링(amplitude scaling) : $y(t) = cx(t), y[n] = cx[n]$
&ensp;덧셈(addition) :$ y(t) = x₁(t) + x₂(t),  y[t] = x₁[t] + x₂[t]$
&ensp;곱셈(multiplication) : $y(t) = x₁(t)x₂(t),  y[t] = x₁[t]x₂[t]$
&ensp;미분(differentiation) : $y(t) =\frac{\mathrm{d} }{\mathrm{d} x}x(t)$
&ensp;시간 스케일링(time scaling) : $y(t) = x(at), y[n] = x[kn]$
&ensp;시간 이동(time shifting) : $y(t) = x(t - t₀)$ (t₀는 시간 이동된 간격, t₀>0 -> 시간축에 대하여 오른쪽, t₀<0 -> 왼쪽으로 이동)

&ensp;기본 신호(Elementary Signals) 
&ensp; 지수함수, 삼각함수, 계단함수, 임펄스 함수, 램프 함수 등
