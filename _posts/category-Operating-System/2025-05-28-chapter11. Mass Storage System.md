...
&ensp;이걸 통해 디스크 컨트롤러(하드웨어)가 여기부터 여기까지 읽자 할 수 있게 됨<br/>
...
* 즉 raw disk access는 운영체제를 거치지 않고 특정 응요 프로그램(앱)이 디스크를 직접 읽고 쓰는 기능이다. 
...
&ensp;컴퓨터가 켜질 때 디스크의 가장 앞 부분에 있는 MBR(Master Boot Record)부터 일고 거기 있는 부트 코드가 어떤 파티션에서 운영체제를 시작할지 결정해주는 구조이다. <br/>

&ensp;🔍 구성요소 설명<br/>

1. 🧠 MBR (Master Boot Record)
* 디그크 맨 앞 512바이트에 있는 아주 중요한 정보
* 두 가지를 가지고 있다.: 
...
&ensp;운영체제는 메인 메모리(RAM)가 부족할 때 디스크 공간의 일부를 RAM처럼 사용하는 공간을 만들어. 이걸 **스왑 공간(Swap Space)**이라고 한다. <br/>
...
2. 메모리와 제어 소프트웨어
* 저장장치를 효유적으로 제어하는 뇌 역할을 하는 부품들이 들어간다. 
...
* RAID: 여러 디스크를 묶어서 속도 증나나 데이터 복구 가능
...
&ensp;스토리지에는 논리적으로 분할된 공간이 있어요 -> LUN(Logical Unit Number)
...
&ensp;Dedup 예시:<br/>
&ensp;회사 서버에 직원들이 같은 회사 로고.png 파일을 저장했다고 해자<br/>
* 직원 A의 폴더: 회사 로고.png
* 직원 B의 폴더: 회사 로고.png
* 직원 C의 폴더: 회사 로고.png
