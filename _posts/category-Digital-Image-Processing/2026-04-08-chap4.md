...
# correspoondence between filtering in spatial domain and frequency domain
...
&ensp;1. convolution property<br/>
&ensp;공간영역에서<br/>

$$g(x, y) = f(x, y) * h(x, y)$$

&ensp;영상 f에 필터 h를 convolution하면 주파수 영역에서는 그냥<br/>

$$G(u, v) = F(u, v)H(u, v)$$

&ensp;처럼 곱셈으로 바뀐다.<br/>
&ensp;공간영역 convolution은 계산량이 크지만 주파수영역에서는 곱셈 한 번으로 처리할 수 있어서 훨씬 효율적일 수 있다.<br/>
* 공간영역: convolution
* 주파수 영역: multiplication

&ensp;2. multiplication property<br/>
&ensp;반대로 공간영역에서 단순 곱셈 $f(x, y)h(x, y)$ 을 하면 주파수영역에서는 $F(u, v) * H(u, v)$ <br/>
&ensp; 즉 convolution이 된다.<br/>
&ensp;이건 convolution property의 반대 방향 관계이다.<br/>
* 공간에서 convolution <-> 주파수에서 곱셈
* 공간에서 곱셈 <-> 주파수에서 convolution

## forward transform 알고리즘으로 inverse transform 계산

&ensp;inverse DFT를 따로 새로 짜지 않아도 conjugate를 이용하면 forward DFT 알고리즘으로 계산할 수 있다.<br/>
...
&ensp;형태가 된다.<br/
&ensp;$F^*(u)$ 를 입력으로 넣고 forward DFT 알고리즘을 돌린 뒤 마지막에 conjugate scaling을 처리하면 inverse transform이 된다.<br/>

&ensp;padding 없이 하면 왜 wrap around error가 생길까?<br/>
<p align="center"><img src="/assets/img/Digital Image Processing/chap4/4-48.png" width="600"></p>

&ensp;1. 에러가 생기는 이유<br/>
&ensp;DFT는 본질적으로 신호를 주기적(periodic)으로 가정한다. 즉 길이가 N인 신호가 있으면 DFT는 그 신호가 한 번만 있는 게 아니라 계속 반복된다고 생각한다.<br/>
&ensp;그래서 주파수 영역에서 곱셈 후 inverse DFT를 하면 얻어지는 것은 보통 linear convolution이 아니라 circular convolution 결과이다.<br/>

&ensp;2. circular convolutio이 문제인 이유<br/>
&ensp;원래 우리가 영상처리에서 원하는 건 보통 linear convolution이다. 즉 필터가 경계를 넘어가면 바깥은 0이라고 생각하는 방식.<br/>
&ensp;그런데 circular convolution은 경계를 넘어간 값이 반대편으로 다시 돌아온다. 그래서 신호 끝 부분이 앞 부분과 겹쳐 버린다.<br/>
&ensp;이게 wrap around<br/>

&ensp;zero padding으로 해결<br/>
<p align="center"><img src="/assets/img/Digital Image Processing/chap4/4-49.png" width="600"></p>

&ensp;길이 A인 신호와 길이 B인 신호를 linear convolution 하면<br/>
&ensp;결과 길이는<br/>

$$A + B -1$$

&ensp;그래서 DFT를 이용해서 같은 결과를 얻고 싶다면 DFT 길이 또는 padding된 총 길이 P를 적어도<br/>

$$Period \geq A + B -1$$

&ensp;로 잡아야 한다.<br/>
&ensp;그러면 circular convolution이더라도 한 주기 안에서는  linear convolution 결과와 같아진다.<br/>

&ensp;zero padding이 필요한 이유<br/>
&ensp;부족한 길이만큼 뒤에 0을 붙이면 주기적으로 감겨 돌아오는 부분이 실제 유효 구간과 겹치지 않게 된다. 즉 넘친 값이 있어도 그게 0 padding 영역에만 들어가니까 우리가 보는 실제 결과 구간은 linear convolution과 같아진다.<br/>

&ensp;circular convolution = linear convolution 이 되는 조건<br/>
&ensp;충분히 padding을 하면 circular convolution 결과가 linear convolution 결과와 동일해진다.<br/>

&ensp;왜 보통 zero padding을 쓸까?<br/>
&ensp;circular convolution 자체는 구현상 다루기 불편하고 실제로는 linear convolution을 원하니까 보통은 **zero padding 후 DFT** 를 사용한다.<br/>

1. 원 신호/영상 뒤에 0을 붙인다.
2. DFT 한다.
3. 주파수영역에서 곱한다.
4. inverse DFT 한다.
5. 그러면 linear convolution 결과를 얻는다.

## zero padding을 이용한 linear convolution

&ensp;두 신호를 N+M-1 길이로 zero padding 한 뒤 DFT를 사용하면 circular convolution 결과가 원래의 linear convolution 결과와 같아진다.<br/>

&ensp;1. 원래 신호 길이<br/>
* f(n): N points
* g(n): M points

&ensp;그리고 이 둘을 convolution 하면 결과 길이는 항상 N + M -1 이 된다.<br/>

&ensp;왜 N+M-1일까?<br/>
&ensp;예를 들어 길이 3짜리와 길이 2짜리를 convolution하면 결과 길이는 3 + 2 -1 = 4가 된다.<br/>

&ensp;2. zero padding된 신호<br/>

$$f(n) \rightarrow f^{'}_c(n)$$
$$g(n) \rightarrow g^{'}_c(n)$$

&ensp;처럼 padding된 신호로 바꾸는 흐름이 그려져 있다. 즉 원래 신호 뒤에 0을 붙여서 길이를 N + M -1 로 맞추는 거다.<br/>
...
&ensp;3. 왜 padding하면 circular convolution이 괜찮아질까?<br/>
&ensp;DFT 기반 계산은 본질적으로 circular convolution이다. 그런데 길이를 충분히 크게 잡아주면 한 주기 안에서 겹쳐 들어오는 부분이 실제 결과 구간을 침범하지 못한다.<br/>
&ensp;그래서

$$f^{'}_c(n) \bigotimes g^{'}_c(n)$$

&ensp;circular convolution 결과가

$$f(n) * g(n)$$

&ensp;즉 linear convolution 결과와 같아진다.<br/>

* ∗: linear convolution
* ⊗: circular convolution

&ensp;예제<br/>
<p align="center"><img src="/assets/img/Digital Image Processing/chap4/4-50.png" width="600"></p>

&ensp;결론<br/>
1. DFT 곱셈 후 inverse DFT는 원래 circular convolution이다.
2. 하지만 신호 길이를 N+M−1로 zero padding 하면 circular convolution 결과가 linear convolution 결과와 같아진다.
3. 그래서 실무에서는 DFT를 이용해 linear convolution을 계산할 수 있다.

## 공간영역 convolution의 계산량

$$y(n) = f(n)*g(n) = \sum_{m=0}^{N-1}f(m)g(n-m)$$

&ensp;of multiplications: $N^2$ <br/>

&ensp;1. 이 식이 뜻하는 것<br/>
&ensp;길이 N짜리 신호 두 개를 직접 convolution한다고 해보자<br/>
&ensp;각 출력 샘플 y(n)를 계산하려면 많은 곱셈과 덧셈이 필요하다. 출력도 대략 N개 정도 생기고 각 출력마다 또 대략 N번 곱해야 하니까 전체 계산량이 대락 $N^2$ 가 된다.<br/>

&ensp;2. 왜 문제일까?<br/>
&ensp;신호 길이가 작으면 괜찮다. 그런데 영상은 픽셀 수가 엄청 많고 필터도 커질 수 있다. 그러면 공간영역에서 직접 convolution 하는 건 계산량이 너무 커진다.<br/>

&ensp;3. 그래서 주파수영역으로 가는 이유<br/>
&ensp;주파수영역에서는 DFT, 곱셈, inverse DFT만 하면 된다. 물론 DFT자체도 계산량이 있지만 FFT를 쓰면 훨씬 줄일 수 있어서 큰 신호에서는 유리해진다.<br/>
sss